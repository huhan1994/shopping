...
$('add').onclick = function(e){
	...
	var productDetail1 = {'id':id,'price':price,'title':title,'num':num};
	var name = 'products';
	var productList1 = new Array;
	var productList = util.getCookie(name);
	if(productList == "" || productList == null){
		productList1.push(productDetail1);
		...
		productList.push(productDetail1);
		util.setCookie(name,productList);
	}
	util.setCookie('product',productDetail);
...
//		util.deleteCookie(name);
	...
	layer.reset({
		...
		onconfirm:function(){
			layer.hide();
			loading.show();
			loading.result('添加购物车成功'); //,location.href="/add"
		}.bind(this)
	}).show();
	return;
};
